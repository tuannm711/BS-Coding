...
test('V2 backend projections and lifecycle commands survive restart', async () => {
  ...
  try {
    ...
    await expect(homeWindow.getByText(/75% remaining/)).toBeVisible()
    ...
    await expect(homeWindow.getByText('10 input tokens', { exact: true })).toBeVisible()
    await expect(homeWindow.getByText('Cost unknown', { exact: true })).toBeVisible()
    await homeWindow.getByRole('button', { name: 'Budget' }).click()
    await homeWindow.getByLabel('Maximum requests').fill('1')
    await homeWindow.getByRole('button', { name: 'Save Budget' }).click()
    await expect(homeWindow.getByText('HARD_BLOCK', { exact: true })).toBeVisible()
