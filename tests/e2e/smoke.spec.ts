...
test('settings agent list immediately reconciles the active workspace', async () => {
  ...
  try {
    ...
    try {
      ...
      await expect(window.locator('.agent-picker-item', { hasText: 'reviewer' })).toBeVisible()
      await window.locator('.agent-picker-item', { hasText: 'reviewer' }).click()
      ...
      const reviewerRow = reopenedSettings.locator('.agents-row').filter({ hasText: 'reviewer' })
