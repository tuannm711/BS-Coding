...
const usage = { costUsd: 9, costKnown: true, inputTokens: 100, requests: 2, concurrentAgents: 1, elapsedMs: 1000 }
...
it('requires approval when a configured cost limit cannot be evaluated', () => {
  expect(evaluateBudget({ maxCostUsd: 10 }, { ...usage, costKnown: false }))
    .toEqual({ decision: 'SOFT_WARNING', metric: 'costUsd', current: 9, limit: 10, unknown: true })
})
