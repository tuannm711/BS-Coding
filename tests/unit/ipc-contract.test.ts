...
describe('IPC contract', () => {
  it('defines all channels used by the preload api', () => {
    const required: (keyof AgentApi)[] = [
      ...
      'listSessions', 'activeSessionFor', 'setAgentRole', 'createSession', 'switchSession', 'deleteSession', 'renameSession',
      'getChatTodos',
      'isChatRunning',
    ...
    const api: AgentApi = {
      ...
      createProviderAuthorization: async () => ({ loginId: '', providerId: '', methodId: 'oauth', authUrl: '', expiresAt: 0, verifier: '', expectedState: '', callbackUrl: '', status: 'waiting' }),
